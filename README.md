...
* `jbd.pemfile` Specifies a source of trusted certificate authorities. Recommended value is: 'https://raw<span></span>.githubusercontent.com/bagder/ca-bundle/master/ca-bundle.crt'. Alternate sources must follow that format too. Can be a local file as well. *Defaults to the JRE keystore*, so you can use JDK's keytool to import specific certs.
