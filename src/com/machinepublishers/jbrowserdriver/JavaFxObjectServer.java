...
class JavaFxObjectServer extends UnicastRemoteObject implements JavaFxObjectRemote {
  ...
  public JavaFxObjectRemote field(String fieldName) {
    ...
    while (curClass != null) {
      try {
        ...
        return ret == null ? null : new JavaFxObjectServer(ret);
      } catch (Throwable t) {
  ...
  public JavaFxObjectRemote call(String methodName, Object... params) throws RemoteException {
    Class[] paramTypes;
    Object[] paramsAlt;
    if (params == null || params.length == 0) {
      ...
      for (int i = 0; i < params.length; i++) {
        params[i] = params[i] instanceof JavaFxObjectRemote ? ((JavaFxObjectRemote) params[i]).unwrap() : params[i];
  ...
  static void unbox(Class[] classes) {
    for (int i = 0; i < classes.length; i++) {
      classes[i] = classes[i] == null ? null
          : (classes[i].equals(Double.class) ? double.class
              : (classes[i].equals(Float.class) ? float.class
